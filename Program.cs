...
assignedSections.ForEach(pair =>
{
    ...
    if (firstElfSections.All(s => secondElfSections.Contains(s)) || secondElfSections.All(s => firstElfSections.Contains(s)))
    {
        completelyOverlappingSectionsCount++;
    }
});
...
assignedSections.ForEach(pair =>
{
    ...
    if (firstElfSections.Any(s => secondElfSections.Contains(s)) || secondElfSections.Any(s => firstElfSections.Contains(s)))
    {
        partiallyOverlappingSectionsCount++;
    }
});
